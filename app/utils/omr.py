...
def calculate_corner_features(corner_img_path):
    ...
    """
    corner_img = cv2.imread(corner_img_path)
    ...
    # the correct corner contour by drawing one contour at a time. Ideally, 
...
def get_corners(contours, corner_features):
...
def get_answers(source_file, corner_img_path):
    ...
    corner_features = calculate_corner_features(corner_img_path)
    corners = get_corners(contours, corner_features)
    ...
    for i, q_patch in enumerate(get_question_patches(normalized_transf)):
        alt_patches = list(get_alternative_patches(q_patch))
        alt_index = get_marked_alternative(alt_patches)
        
        letter = get_letter(alt_index)
